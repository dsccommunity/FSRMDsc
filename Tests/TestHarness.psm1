function Invoke-TestHarness
{
    ...
    Write-Verbose -Message 'Commencing all FSRMDsc tests'

    $repoDir = Join-Path -Path $PSScriptRoot -ChildPath '..\' -Resolve

    $testCoverageFiles = @()
    Get-ChildItem -Path "$repoDir\modules\FSRMDsc\DSCResources\**\*.psm1" -Recurse | ForEach-Object {
    ...
    Import-Module -Name "$repoDir\modules\FSRMDsc\FSRMDsc.psd1"
    $testsToRun = @()
